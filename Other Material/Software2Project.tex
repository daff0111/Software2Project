...
\item \textbf{Registered User/Users:} A User that has provided valid credentials and payment informations to create an Account in the System.
...
\item [G.10)]Users must be able to finish their use of the Car when leaving it in a Safe Parking Area and exiting the car. The User will then be charged for the use of the service. The used Car will be locked and freed for Users to be reserved.
\begin{itemize}
	\item[-]The system must verify if the car is parked in a Safe Parking Area. If the car isn\textsc{\char13}t located in it the system must notify the user on the screen of the car that it\textsc{\char13}s not possible to finish the ride.
	\item[-]The system must show a message on the car\textsc{\char13}s screen asking if the user want to finish the ride or if want to continuing the use of the car.
...
\begin{itemize}
	\item[-]The system must notify the user on the car’s screen of the any information related with a Transaction.
\end{itemize}
...
\hline
	...
	\textbf{Exit conditions} &\parbox[t]{8cm}{ System confirms the creation of a new user and send an email to the user\textsc{\char13}s email address informing the password and confirming the registration.}\\ \hline
	\textbf{Exception} & \parbox[t]{8cm}{Carol inserts data that the system can not validate. System can not finish operation until Carol inserts the correct data.   }\\ \hline
\end{tabular}
...
\hline
	...
	\textbf{Flux of events} & \parbox[t]{8cm}{\textbullet  Francesco open the app and login;\\\textbullet Francesco try to find a car manually searching in the map, but he does not like of searching by himself;\\\textbullet Francesco open the side menu and find the option ‘Search for a car’, he selects it;\\\textbullet The system automatically search for the nearest car based on Francesco\textsc{\char13}s location;\\\textbullet 	The system find the finds a car and trace the route in the map of the App\textsc{\char13}s map;\\\textbullet  Francesco satisfied to know this functionality of the app and closes it without reserving a car; }
  \\ \hline
...
\hline
	...
	\textbf{Flux of events} & \parbox[t]{8cm}{\textbullet  Sarah open the App and do the login;\\\textbullet Sarah selects in the menu of the App the option \textsc{\char13}My reservation\textsc{\char13};\\\textbullet The system shows Sarah’s reservation;\\\textbullet Sarah see the status of the reservation;\\\textbullet Sarah selects the option of canceling the reservation;\\\textbullet The system checks the time that Sarah did the reservation, verifies that is possible to cancel the ride and proceeds to do this operation;} \\ \hline
...
Paulo couldn\textsc{\char13}t make to the car in 15 minutes and the system automatically cancel the reservation, charges 1 euro of Paulo\textsc{\char13}s account and will not allow Paulo to reserve a car for 1 hour.
}\\ \hline
\end{tabular}
...
\hline
	...
	\textbf{Exit conditions} &\parbox[t]{8cm}{ The system verifies the Status of the car and   proceeds to finish Marina\textsc{\char13}s ride sending to his email the information of the ride as the route and the transaction.}\\ \hline
