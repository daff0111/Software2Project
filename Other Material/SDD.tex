...
Following our Architecture we organize the High level components on the 3 Tiers. As this is not yet the Deployment view this components do not represent the actual distribution, it shows the interaction between them and what they represent inside the System. Clients will have their User and CRM Components, distributed part in the Client as Interface for the App, and part in the Server as Service Requestors. Inside the Server through Session Beans a Session Manager will provide the services to the clients by communicating with the respective Controllers. Each Controller will manage and represent the System Entities previously explained in the RASD and will interface with a Persistent DataBase Connection to the DBMS. Helpers and Extra Feature Components are represented as well.
...
\begin{itemize}
\item User and CRM Components: Implement the Sessions for each Actor (App) and will have ServiceRequestors to interface with the central Server Component.
\item Session Manager: Will manage sessions and forward requests to the correspondant Controller.
